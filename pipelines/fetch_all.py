...
# ---------- 1) TheOddsAPI (sin forzar bet365) ----------
def fetch_theoddsapi(hours_ahead=72):
    ...
    try:
        ...
        print(f"theoddsapi: error list sports: {e}"); sports = []

    keep = [s for s in sports if (s.get("key","").startswith(("soccer_","americanfootball_","basketball_","baseball_","icehockey_","tennis_","esports_","tabletennis_","mma_","boxing_","motorsport_")))]
    rows=[]; end_time = utcnow() + timedelta(hours=hours_ahead)
    for s in keep:
        skey = s["key"]
        try:
            r = requests.get(f"https://api.the-odds-api.com/v4/sports/{skey}/odds",
            ...
            print(f"theoddsapi: {skey} fail {e}"); continue

        for ev in events:
            try: start = parser.isoparse(ev["commence_time"])
            except Exception: continue
            if start <= utcnow() or start > end_time: continue

            home, away = ev.get("home_team",""), ev.get("away_team","")
            league = s.get("title", skey)
            sk = skey.split("_")[0]
            sport = {"soccer":"futbol","americanfootball":"americano","basketball":"baloncesto","baseball":"beisbol","icehockey":"hockey","tennis":"tenis","esports":"esports","tabletennis":"ping_pong"}.get(sk, sk)
            ...
            rows.append(dict(sport=sport, league=league, start_time_utc=iso(start), status="scheduled",
...
def fetch_soccer_apifootball(hours_ahead=72):
    ...
    start=utcnow(); end=start+timedelta(hours=hours_ahead)
    rows=[]
    try:
        ...
        print(f"apifootball fail: {e}"); data=[]
    for fx in data:
        st=(safe_get(fx,"fixture","status","short", default="") or "").upper()
        if st in ("FT","AET","PEN","PST","CANC"): continue
        date=safe_get(fx,"fixture","date"); 
        if not date: continue
        try: dt=parser.isoparse(date)
        except Exception: continue
        if dt <= utcnow(): continue
        ...
        rows.append(dict(sport="futbol", league=league, start_time_utc=iso(dt), status="scheduled",
...
def fetch_nba_balldontlie(hours_ahead=72):
    start=utcnow().date(); end=(utcnow()+timedelta(hours=hours_ahead)).date()
    rows=[]; page=1
    while True:
        try:
        ...
        if not data: break
        for g in data:
            if g.get("status") in ("Final","Postponed","Canceled"): continue
            try: dt=parser.isoparse(g.get("date"))
            except Exception: continue
            home=safe_get(g,"home_team","full_name", default="Home")
            away=safe_get(g,"visitor_team","full_name", default="Away")
            rows.append(dict(sport="baloncesto", league="NBA", start_time_utc=iso(dt), status="scheduled",
                             home=home, away=away, market_total=None, ml_home=None, ml_away=None,
                             spread_line=None, spread_home=None, spread_away=None))
        page+=1
        if page>12: break
...
def fetch_mlb_statsapi(hours_ahead=72):
    start=utcnow().date().isoformat(); end=(utcnow()+timedelta(hours=hours_ahead)).date().isoformat()
    rows=[]
    try:
        data=requests.get(f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&startDate={start}&endDate={end}", timeout=30).json()
...
def fetch_nhl_statsapi(hours_ahead=72):
    ...
    rows=[]
    try:
        data=requests.get(f"https://statsapi.web.nhl.com/api/v1/schedule?startDate={start}&endDate={end}", timeout=30).json()
        for day in data.get("dates",[]):
            for g in day.get("games",[]):
                if g.get("status",{}).get("statusCode")=="7": continue  # Final
                home=g.get("teams",{}).get("home",{}).get("team",{}).get("name")
                away=g.get("teams",{}).get("away",{}).get("team",{}).get("name")
                dt=parser.isoparse(g.get("gameDate"))
...
def fetch_nfl_espn(hours_ahead=96):
    ...
    for off in range(0, int(hours_ahead/24)+2):
        ...
        try:
            r=requests.get("https://site.api.espn.com/apis/v2/sports/football/nfl/scoreboard", params={"dates": day}, timeout=30)
            if r.status_code!=200: continue
            for ev in r.json().get("events", []):
                comps=safe_get(ev,"competitions",0, default={})
                status=(safe_get(comps,"status","type","name", default="") or "").lower()
                if status in ("status_final","final","postponed","canceled"): continue
                dt_txt=safe_get(comps,"date", default=None)
                if not dt_txt: continue
                try: dt=parser.isoparse(dt_txt)
                except Exception: continue
                if dt <= utcnow(): continue
...
# ---------- 6) PandaScore ----------
def fetch_pandascore(hours_ahead=96):
    if not PANDASCORE_TOKEN:
        return pd.DataFrame(columns=["sport","league","start_time_utc","status","home","away"])
    end=(utcnow()+timedelta(hours=hours_ahead)).isoformat(); start=utcnow().isoformat()
    rows=[]; page=1
    while True:
        try:
            r=requests.get("https://api.pandascore.co/matches/upcoming",
        ...
        for m in data:
            ...
            if not dt or dt <= utcnow(): continue
...
if __name__=="__main__":
    parts=[fetch_theoddsapi(72), fetch_soccer_apifootball(72), fetch_nba_balldontlie(72),
           fetch_mlb_statsapi(96), fetch_nhl_statsapi(96), fetch_nfl_espn(120), fetch_pandascore(96)]
    frames=[p for p in parts if isinstance(p,pd.DataFrame)]
    df=pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=["sport","league","start_time_utc","status","home","away","market_total","ml_home","ml_away","spread_line","spread_home","spread_away"])
    if not df.empty:
    ...
    out=PROC/"upcoming_events.csv"; df.to_csv(out, index=False)
    if not df.empty:
